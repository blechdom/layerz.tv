...
//, {
 //     		prepend: false
  ...
  socket.on('foregroundStreamStatus', function (data) {
    console.log('foreground stream status ' + data.streamRunning + ' : ' + data.paused);
    if (data.streamRunning) {
      playerInstance.load([{file:"rtmp://danmnet.ucsc.edu/live/layerz"}]);
    } else {
      playerInstance.load([{file:"/foreground/giant%20bug.mp4"}]);
    }
    if (data.paused) {
      pause = true;
  ...
  socket.on('nextForegroundPlaying', function (data) {
    console.log('next foreground playing ');
    if (selection) {
  ...
  socket.on('changeForegroundStream', function (data) {
    console.log('change foreground stream ' + data);
    if (data == 'start') {
  ...
  socket.on('pumpLog', function (data) {
    ...
    if (!selection && !pause) {
      socket.emit('recieveForegroundVote', this.src);
  ...
  playerInstance.setup({
    file: "/foreground/giant%20bug.mp4",
    controls: false,
    autostart : true,
    autoplay : true,
    repeat : true,
  ...
  socket.emit('foregroundStreamStatus', '');
});
